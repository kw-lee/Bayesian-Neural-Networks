...
if args.model == 'Local_Reparam':
    net = BBP_Bayes_Net_LR(
        lr=lr, channels_in=1, side_in=28, cuda=use_cuda, classes=10, 
        batch_size=batch_size, Nbatches=(NTrainPointsMNIST / batch_size), 
        n_hid=n_hid, prior_sig=args.prior_sig)
elif args.model == 'Laplace_prior':
    net = BBP_Bayes_Net(
        lr=lr, channels_in=1, side_in=28, cuda=use_cuda, classes=10, 
        batch_size=batch_size, Nbatches=(NTrainPointsMNIST / batch_size), 
        n_hid=n_hid, prior_instance=priors.laplace_prior(mu=0, b=args.prior_sig))
elif args.model == 'Gaussian_prior':
    net = BBP_Bayes_Net(
        lr=lr, channels_in=1, side_in=28, cuda=use_cuda, classes=10, 
        batch_size=batch_size, Nbatches=(NTrainPointsMNIST / batch_size), 
        n_hid=n_hid, prior_instance=priors.isotropic_gauss_prior(mu=0, sigma=args.prior_sig))
elif args.model == 'GMM_prior':
    net = BBP_Bayes_Net(
        lr=lr, channels_in=1, side_in=28, cuda=use_cuda, classes=10, 
        batch_size=batch_size, Nbatches=(NTrainPointsMNIST / batch_size), 
        n_hid=n_hid,
        prior_instance=priors.spike_slab_2GMM(mu1=0, mu2=0, sigma1=args.prior_sig, sigma2=0.0005, pi=0.75))
else:
...
for i in range(epoch, nb_epochs):
    ...
    if i % nb_its_dev == 0:
        ...
        utils.cprint('g', '    Jdev = %f, err = %f\n' % (cost_dev[i], err_dev[i]))

        if err_dev[i] < best_err:
            best_err = err_dev[i]
            utils.cprint('b', 'best test error')
            net.save(models_dir + '/theta_best.dat')

toc0 = time.time()
runtime_per_it = (toc0 - tic0) / float(nb_epochs)
utils.cprint('r', '   average time: %f seconds\n' % runtime_per_it)
...
print('  nb_parameters: %d (%s)' % (nb_parameters, utils.humansize(nb_parameters)))
